const sampleProducts = [
  ...
  function getProductsByCategory(products, category) {
    return products.filter((kategori) => kategori.category === category);
    /**
     * TODO:
  ...
  function findProductById(products, id) {
    return products.filter((tag) => tag.id === id).find((tag)=> tag.id === id);
      /**
       * TODO:
    ...
    function calculateTotalPrice(products) {
      return products.reduce((acc,id) => acc + id.price,0);
    /**
     * TODO:
